...
export default function BusinessRegistrationPanel({
  ...
  const [documents, setDocuments] = useState<DocumentItem[]>([
  {
    ...
    status: 'Required',
    description:
    'Primary business operation permit issued by the local government',
  ...
  },
  {
    ...
    status: 'N/A',
    description:
    'Social Security System employer registration (if hiring employees)',
  ...
  const totalFees = requiredDocuments.reduce(
    (sum, doc) => sum + parseInt(doc.fee.replace(/[^0-9]/g, '')),
    0
  );
  const paidFees = completedDocuments.
  filter((doc) => doc.status === 'Required').
  reduce((sum, doc) => sum + parseInt(doc.fee.replace(/[^0-9]/g, '')), 0);
  ...
  businessName.trim() !== '' &&
  ownerName.trim() !== '' &&
  requiredDocuments.every((doc) => doc.completed);
            ...
            <div className="min-h-full p-6 md:p-8">
              ...
              className="bg-m2m-bg-card rounded-2xl p-6 mb-8 border border-m2m-divider shadow-lg">
                ...
                <motion.div
                  ...
                  className={`bg-m2m-bg-primary rounded-xl p-4 cursor-pointer transition-all ${doc.completed ? 'border-2 border-m2m-success bg-m2m-success/5' : 'border border-m2m-divider hover:border-m2m-accent-blue'}`}
                  onClick={() => toggleDocumentComplete(doc.id)}>
                      ...
                      <div className="w-6 h-6 rounded-full border-2 border-m2m-divider" />
                      }
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-start justify-between gap-4 mb-2">
                            ...
                            <div className="flex items-center gap-3 flex-shrink-0">
                              <span
                            className={`px-3 py-1 rounded-full text-xs font-semibold ${doc.status === 'Required' ? 'bg-m2m-accent-blue/20 text-m2m-accent-blue border border-m2m-accent-blue/40' : 'bg-m2m-bg-primary text-m2m-text-secondary border border-m2m-divider'}`}>
              ...
              className="bg-m2m-bg-card rounded-2xl p-8 text-center border border-m2m-divider shadow-lg">
                ...
                className={`w-full md:w-auto px-8 py-4 rounded-xl font-semibold text-lg transition-all ${isFormComplete ? 'bg-gradient-to-r from-m2m-accent-blue to-m2m-accent-teal text-white shadow-lg hover:shadow-xl' : 'bg-m2m-bg-primary text-m2m-text-secondary cursor-not-allowed opacity-50'}`}>
