...
function AutoCenterMap({ center }: {center: [number, number] | null;}) {
  const map = useMap();
  useEffect(() => {
    if (center) {
      map.flyTo(center, 13, {
...
const generateAIAnalysis = (businessType: string, locationId: string) => {
  const seed = (businessType + locationId).
  split('').
  reduce((acc, char) => acc + char.charCodeAt(0), 0);
  ...
  return {
    ...
    competitorLevel:
    competitors <= 3 ? 'Low' : competitors <= 6 ? 'Medium' : 'High',
...
trend: string) =>
{
  ...
  competitors <= 3 ?
  `minimal competition (only ${competitors} similar businesses)` :
  competitors <= 6 ?
  `moderate competition (${competitors} similar businesses)` :
  `high competition (${competitors} similar businesses)`;
...
export default function UnifiedMapView() {
  ...
  const { updateUserLocation, isLocationSharing, toggleLocationSharing } =
  useLocationSharing();
  ...
  const [locationInsights, setLocationInsights] =
  useState<LocationInsights | null>(null);
  ...
  const [contactSupplier, setContactSupplier] =
  useState<SupplierDetails | null>(null);
  const [usedSupplierNames, setUsedSupplierNames] = useState<Set<string>>(
    new Set()
  );
  const [mapCenter, setMapCenter] =
  useState<[number, number]>(TANDAG_CITY_CENTER);
  ...
  const [showBusinessRegistrationPanel, setShowBusinessRegistrationPanel] =
  useState(false);
  ...
  const zones = [
  {
    ...
    aiScore: 92,
    description:
    'Main business hub around Rizal Street with shops, banks, and government offices'
  },
  {
    ...
    aiScore: 0,
    description:
    'Mabua and Awasian residential areas with growing communities'
  },
  {
    ...
    aiScore: 85,
    description:
    'Bongtud and Pandanon areas with residential and commercial mix'
  },
  {
  ...
  },
  {
    ...
    aiScore: 88,
    description:
    'Coastal recreation zone with beach resorts and tourism facilities'
  }];
  ...
  const CATEGORY_PRODUCTS: Record<string, string[]> = {
    ...
    'Professional Price Tag Gun',
    'Display Mannequins Set',
  ...
  const isOnLand = (lat: number, lng: number): boolean => {
    ...
    const isInPhilippinesBounds =
    lat >= 4.5 && lat <= 21 && lng >= 116 && lng <= 127;
  ...
  const generateUniqueProducts = (supplierId: string): string[] => {
    ...
    selectedCategories.forEach((category, catIndex) => {
      ...
      for (
      let i = 0;
      i < productsFromCategory && products.length < productCount;
      i++)
      {
  ...
  : SupplierDetails[] => {
    ...
    const availableNames = allSupplierNames.filter(
    ...
    if (availableNames.length < count) {
      ...
      for (let i = 0; i < adjustedCount; i++) {
        ...
        const position = generateStablePosition(
          supplierId,
          i,
          lat,
          lng,
        ...
        const type =
        supplierTypes[Math.floor(Math.random() * supplierTypes.length)];
        ...
        const servicesOffered = allServices.
  ...
  useEffect(() => {
    ...
    const initialSuppliers = generateUniqueSuppliers(
      tandagCenter[0],
      tandagCenter[1],
      5,
      10
  ...
  const fetchLocationInsights = async (lat: number, lng: number) => {
    ...
    const mockData: LocationInsights = {
      ...
      regulations:
      'General city ordinances apply. Commercial activities permitted with proper permits.',
      requiredPermits: [
      ...
      demographics: {
        ...
        consumerBehavior: {
          spendingPatterns:
          'High retail spending, moderate dining out frequency',
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        if (!isOnLand(location.lat, location.lng)) {
          ...
          toast.warning(
            'Location adjusted to nearest land area (GPS detected ocean coordinates)',
            {
              duration: 4000
            }
          );
        ...
        setDisplayedSuppliers(initialSuppliers);
        toast.success(
          'Location tracking enabled! Map centered on your position.',
          {
            duration: 4000
          }
        );
      ...
      (error) => {
        ...
        if (error.code === error.PERMISSION_DENIED) {
          toast.error(
            'Location permission denied. Using Tandag City as default.',
            {
              duration: 5000
            }
          );
          ...
          toast.error(
            'Location information unavailable. Using Tandag City as default.'
          );
        } else if (error.code === error.TIMEOUT) {
          toast.error(
            'Location request timed out. Using Tandag City as default.'
          );
        } else {
          toast.error(
            'Unable to retrieve location. Using Tandag City as default.'
          );
        }
      },
      {
  ...
  const handleLocationUpdate = (lat: number, lng: number) => {
    ...
    if (showNearbyOperators) {
      const newSuppliers = generateUniqueSuppliers(
  ...
  const handleShopRedirect = (supplierId: string) => {
    const supplier = stableSupplierPositions.find(
  ...
  const handleMapClick = (lat: number, lng: number) => {
    ...
    if (showNearbyOperators) {
      ...
      setDisplayedSuppliers(newSuppliers);
      toast.success(
        `Location pinned! Discovering ${newSuppliers.length} nearby suppliers...`
      );
    }
  };
  ...
  return (
    <div className="space-y-6 pt-6 relative">
          ...
          <div className="flex items-center justify-between p-4 nearbuy-card rounded-lg">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                checked={showZoneInfo}
                onChange={() => setShowZoneInfo(!showZoneInfo)} />
          ...
          <div className="flex items-center justify-between p-4 nearbuy-card rounded-lg ambient-glow">
            <div>
              <p className="font-medium text-m2m-text-primary flex items-center">
                Nearby Operators
                {showNearbyOperators && isTrackingLocation &&
                <motion.span
                  animate={{
                    opacity: [1, 0.5, 1]
                  }}
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                type="checkbox"
                className="sr-only peer"
                checked={showNearbyOperators}
                onChange={handleNearbyOperatorsToggle} />
              ...
              <motion.div
                ...
                {
                  boxShadow: [
                  '0 0 0 0 rgba(124, 255, 138, 0.7)',
                  '0 0 0 10px rgba(124, 255, 138, 0)']
          ...
          <div className="flex items-center justify-between p-4 nearbuy-card rounded-lg">
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                onChange={() => setShowAIRecommender(!showAIRecommender)} />
          ...
          {zoneClassifications.map((zone, index) => {
            ...
            return (
              <motion.div
                ...
                <div
                  className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 shadow-nearbuy-sm"
                  style={{
          ...
          <div>
            ...
            <div className="relative group">
              ...
              <input
                ...
                placeholder="Search for areas in Tandag City…"
                className="w-full pl-10 pr-4 py-3 bg-m2m-bg-primary border border-m2m-divider rounded-lg text-m2m-text-primary placeholder-m2m-text-secondary focus:outline-none focus:border-m2m-nearbuy-teal focus:ring-2 focus:ring-m2m-nearbuy-teal/20 transition-all" />
          ...
          <div>
            ...
            <div className="relative group">
              <select
                ...
                className="w-full px-4 py-3 bg-m2m-bg-primary border border-m2m-divider rounded-lg text-m2m-text-primary appearance-none focus:outline-none focus:border-m2m-nearbuy-teal focus:ring-2 focus:ring-m2m-nearbuy-teal/20 transition-all cursor-pointer">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2">
          <motion.div
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-xs text-m2m-text-secondary mt-1">
                  {isTrackingLocation ?
                  'Tracking your live location. Click anywhere to pin a location for insights.' :
                  'Default view: Tandag City (9.0761° N, 126.1967° E). Click "Track Location" to use your GPS or click anywhere to analyze a location.'}
                </p>
              </div>
              <div className="flex items-center space-x-2">
                <motion.button
                  ...
                  className={`px-4 py-2 rounded-lg transition-all flex items-center text-sm font-medium button-press ${isRequestingLocation ? 'bg-m2m-divider text-m2m-text-secondary cursor-not-allowed' : isTrackingLocation ? 'btn-nearbuy text-white shadow-nearbuy-md' : 'nearbuy-card hover:shadow-nearbuy-sm'}`}>
                ...
                <motion.button
                  ...
                  className="p-2 nearbuy-card rounded-lg transition-all flex items-center text-sm text-m2m-text-secondary hover:text-m2m-nearbuy-teal button-press">
            ...
            <div
              id="map-container"
              className="relative rounded-lg overflow-hidden shadow-nearbuy-xl border-2 border-m2m-nearbuy-green/30 hover:border-m2m-nearbuy-teal/50 transition-all"
              style={{
                height: '600px',
                zIndex: 1
              }}>
              ...
              <MapContainer
                    ...
                    weight: 2
                        ...
                        <Popup>
                          <div className="p-2 min-w-[200px]">
                            ...
                            {zone.aiScore > 0 && showAIRecommender &&
                ...
                <>
                      <Marker
                    position={[9.076, 126.197]}
                    icon={aiRecommendedIcon}>
                  ...
                  {showNearbyOperators &&
                selectedLocation &&
                stableSupplierPositions.map(({ supplier, position }) =>
        ...
        <div className="lg:col-span-1">
          <motion.div
            ...
            <p className="text-sm text-m2m-text-secondary mb-4">
              {selectedLocation ?
              'Based on your pinned location' :
              'Suppliers in your area'}
            </p>
            <AnimatePresence mode="wait">
              {showNearbyOperators ?
              <motion.div
                key={displayedSuppliers.map((s) => s.id).join('-')}
                initial={{
                  opacity: 0,
                  y: 20
                }}
                ...
                className="space-y-3 mb-4 max-h-[500px] overflow-y-auto">
                  ...
                  {stableSupplierPositions.map(({ supplier }, index) =>
                <motion.div
                  key={supplier.id}
                  initial={{
                    opacity: 0,
                  ...
                  className="nearbuy-card rounded-lg p-4 hover-lift shadow-nearbuy-sm hover:shadow-nearbuy-md cursor-pointer">
                      ...
                      <span
                        key={idx}
                        className="text-xs px-2 py-0.5 bg-nearbuy-soft text-m2m-nearbuy-teal rounded-full border border-m2m-nearbuy-teal/20">
                      ...
                      className="px-3 py-2 btn-nearbuy text-white rounded-lg text-sm font-medium shadow-nearbuy-sm hover:shadow-nearbuy-md transition-all duration-300 flex items-center justify-center button-press">
                      ...
                      className="px-3 py-2 nearbuy-card text-m2m-nearbuy-teal rounded-lg text-sm font-medium shadow-nearbuy-sm hover:shadow-nearbuy-md transition-all duration-300 flex items-center justify-center button-press">
          ...
          <motion.div
            ...
            className="glass-panel rounded-xl shadow-nearbuy-xl p-6 border-2 border-m2m-nearbuy-green/30 mt-6 ambient-glow"
            style={{
              background:
              'linear-gradient(135deg, rgba(58, 166, 107, 0.1) 0%, rgba(31, 111, 179, 0.1) 100%)'
            }}>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <div className="bg-m2m-bg-primary rounded-lg p-4 border border-m2m-divider space-y-3">
                      ...
                      <div className="flex justify-between items-center">
                        ...
                        <span
                        className={`text-xs font-bold px-3 py-1 rounded-full border ${getHazardBadgeColor(locationInsights.zoneDetails.hazardLevel)}`}>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="bg-m2m-bg-primary rounded-lg p-4 border border-m2m-divider">
                      <ul className="space-y-2">
                        {locationInsights.requiredPermits.map(
                        (permit, index) =>
                        <li
                          key={index}
                          className="flex items-start text-sm text-m2m-text-secondary">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {locationInsights.nearbyBusinesses.map(
                      (business, index) =>
                      <div
                        key={index}
                        className="bg-m2m-bg-primary rounded-lg p-4 border border-m2m-divider hover:border-m2m-accent-blue transition-all">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {locationInsights.swot.strengths.map((item, index) =>
                  <div
                    key={index}
                    className="bg-m2m-success/10 border border-m2m-success/30 rounded-lg px-3 py-2">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {locationInsights.swot.weaknesses.map((item, index) =>
                  <div
                    key={index}
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {locationInsights.swot.opportunities.map((item, index) =>
                  <div
                    key={index}
                    className="bg-m2m-accent-blue/10 border border-m2m-accent-blue/30 rounded-lg px-3 py-2">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {locationInsights.swot.threats.map((item, index) =>
                  <div
                    key={index}
                    className="bg-m2m-accent-orange/10 border border-m2m-accent-orange/30 rounded-lg px-3 py-2">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {locationInsights.demographics.ageDistribution.map(
                      (age, index) =>
                      <div key={index}>
                            <div className="flex justify-between items-center mb-2">
                              <span className="text-sm text-m2m-text-secondary">
                    ...
                    <div className="space-y-3">
                      <div className="bg-m2m-bg-primary rounded-lg p-4 border border-m2m-divider">
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-sm font-bold text-m2m-text-primary">
                            {
                          locationInsights.demographics.economicProfile.
                          avgIncome
                          }
                          </span>
                        </div>
                      ...
                      <div className="bg-m2m-bg-primary rounded-lg p-4 border border-m2m-divider">
                        <div className="flex items-start space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-m2m-text-secondary">
                              {
                            locationInsights.demographics.consumerBehavior.
                            spendingPatterns
                            }
                            </p>
                          </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="w-full h-[400px] flex flex-col bg-m2m-bg-card rounded-lg p-6 border border-m2m-divider">
                  ...
                  <div className="space-y-4 flex-1 overflow-y-auto">
                    {businessRecommendations.map((business, index) =>
                  <motion.div
                    key={business.name}
                    initial={{
                      opacity: 0,
              ...
              <motion.div
                ...
                className="bg-gradient-to-br from-m2m-accent-blue/5 to-m2m-accent-teal/5 rounded-xl p-6 border border-m2m-divider hover:border-m2m-accent-blue/50 transition-all">
                    ...
                    <TrendingUpIcon className="w-5 h-5 text-m2m-success" />
                          ...
                          <span
                      className={`text-xs font-bold px-3 py-1 rounded-full ${analysis.trendDirection === 'up' ? 'bg-m2m-success/20 text-m2m-success' : 'bg-m2m-chart-yellow/20 text-m2m-chart-yellow'}`}>
                    ...
                    className="bg-m2m-bg-card rounded-lg p-4 border border-m2m-divider text-center">
                          ...
                          <span
                      className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${analysis.populationLevel === 'High' ? 'bg-m2m-success/20 text-m2m-success' : analysis.populationLevel === 'Medium' ? 'bg-m2m-accent-blue/20 text-m2m-accent-blue' : 'bg-m2m-chart-yellow/20 text-m2m-chart-yellow'}`}>
                    ...
                    className="bg-m2m-bg-card rounded-lg p-4 border border-m2m-divider text-center">
                          ...
                          <span
                      className={`text-xs px-2 py-0.5 rounded-full mt-1 inline-block ${analysis.competitorLevel === 'Low' ? 'bg-m2m-success/20 text-m2m-success' : analysis.competitorLevel === 'Medium' ? 'bg-m2m-chart-yellow/20 text-m2m-chart-yellow' : 'bg-red-500/20 text-red-500'}`}>
            ...
            className="bg-gradient-to-br from-m2m-accent-blue/5 to-m2m-accent-teal/5 rounded-xl shadow-lg p-8 border border-m2m-divider">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-4xl mx-auto">
                ...
                className="bg-m2m-bg-card rounded-xl p-6 border border-m2m-divider hover:border-m2m-accent-blue transition-all duration-300 shadow-lg hover:shadow-xl">
                  ...
                  onClick={() => setShowBusinessRegistrationPanel(true)}
                  className="w-full px-6 py-3 bg-gradient-to-r from-m2m-accent-blue to-m2m-accent-teal text-white rounded-lg font-semibold hover:shadow-lg transition-all duration-300 flex items-center justify-center">
      ...
      <AnimatePresence>
        ...
        <motion.div
          ...
          onClick={() => setSelectedSupplier(null)}>
            ...
            className="glass-panel rounded-2xl shadow-nearbuy-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto border-2 border-m2m-nearbuy-green/30"
            onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6">
                <div className="flex items-start justify-between mb-6">
                  ...
                  onClick={() => setSelectedSupplier(null)}
                  className="p-2 nearbuy-card rounded-lg transition-all">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex space-x-3 pt-4">
                    ...
                    onClick={() => handleShopRedirect(selectedSupplier.id)}
                    className="flex-1 px-6 py-3 btn-nearbuy text-white rounded-lg font-semibold shadow-nearbuy-md hover:shadow-nearbuy-lg transition-all duration-300 flex items-center justify-center button-press">
      ...
      {/* Contact Modal */}
      <AnimatePresence>
        {showContactModal && contactSupplier &&
        <motion.div
          initial={{
            opacity: 0
          }}
          ...
          onClick={() => {
            ...
            className="bg-m2m-bg-card rounded-2xl shadow-2xl max-w-md w-full border border-m2m-divider"
            onClick={(e) => e.stopPropagation()}>
              ...
              <div className="p-6">
                ...
                <form className="space-y-4">
                  <div>
                    ...
                    <input
                    type="text"
                    className="w-full px-4 py-3 bg-m2m-bg-primary border border-m2m-divider rounded-lg text-m2m-text-primary focus:outline-none focus:border-m2m-accent-blue transition-colors"
                  ...
                  <div>
                    ...
                    <textarea
                    rows={4}
                    className="w-full px-4 py-3 bg-m2m-bg-primary border border-m2m-divider rounded-lg text-m2m-text-primary focus:outline-none focus:border-m2m-accent-blue transition-colors resize-none"
      ...
      <AnimatePresence>
        ...
        <motion.div
          ...
          className="fixed inset-0 bg-black/60 backdrop-blur-md z-50 flex items-center justify-center p-4"
          onClick={() => setShowBusinessRegistrationPanel(false)}>
              ...
              <div className="p-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex space-x-3 pt-4">
                    ...
                    onClick={() => setShowBusinessRegistrationPanel(false)}
                    className="flex-1 px-6 py-3 nearbuy-card text-m2m-nearbuy-teal rounded-lg font-semibold shadow-nearbuy-sm hover:shadow-nearbuy-md transition-all duration-300 flex items-center justify-center">
